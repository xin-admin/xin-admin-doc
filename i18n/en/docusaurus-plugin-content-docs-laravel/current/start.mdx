...
:::info
在前后端分离开发的模式下，我们建议你保持本地环境作为开发环境，并通过 `php artisan serve` 和 `pnpm run dev` 来启动本地项目作为开发环境，如果需要部署请参考`部署环节`
:::

:::warning
在服务器端使用 `php artisan serve` 和 `pnpm run dev` 启动项目会造成服务器性能浪费，请打包前端代码，并使用 nginx 或者 apache 来部署。具体参考`部署环节`
:::
